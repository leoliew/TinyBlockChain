...
describe('Transaction class', function () {
  describe('构造方法', function () {
    ...
    it('创建的交易流水信息正确', function () {
      ...
      expect(txObject.fromAddress).toBe('a1')
      expect(txObject.toAddress).toBe('b1')
      expect(txObject.amount).toBe(10)
    })
  })
  ...
  describe('计算 hash', function () {
    it('可以正确计算交易流水的 SHA256', function () {
      ...
      expect(txObject.calculateHash()).toBe('21894bb7b0e56aab9eb48d4402d94628a9a179bc277542a5703f417900275153')
    })
    ...
    it('交易篡改会产生不一样的 hash', function () {
      ...
      txObject.amount = 100
      expect(txObject.calculateHash()).not.toBe(originalHash)
    })
  })
  ...
  describe('交易是否有效', function () {
    ...
    it('可以对交易流水签名', function () {
      ...
      expect(txObject.signature).toBe('3044022023fb1d818a0888f7563e1a3ccdd68b28e23070d6c0c1c5004721ee1013f1d769022037da026cda35f95ef1ee5ced5b9f7d70e102fcf841e6240950c61e8f9b6ef9f8')
    })
    ...
    it('可以检测签名错误的交易', function () {
      ...
      txObject.amount = 100
      expect(txObject.isValid()).toBe(false)
    })

    it('正确签名，校验有效性会返回true', function () {
      txObject = createSignedTx()
      expect(txObject.isValid()).toBe(true)
    })
